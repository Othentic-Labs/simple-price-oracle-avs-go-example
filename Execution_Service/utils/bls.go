...
// GetSigningKey takes a hex-encoded seed and returns the Fr element used as a private signing key
// func GetSigningKey(seed string) (*fr.Element, error) {
// 	if len(seed) < 2 || seed[:2] != "0x" {
// 		seed = "0x" + seed
// 	}
...
func GetSigningKey(hexStr string) (fr.Element, error) {
	// Make sure the string is prefixed with 0x
	var prefixed string
	...
	// Hash the string (as bytes, not as hex-decoded bytes)
	hash := sha256.Sum256([]byte(prefixed))

	var sk fr.Element
	if sk.SetBytes(hash[:]) == nil {
		return fr.Element{}, fmt.Errorf("failed to set bytes for signing key")
	}
	return sk, nil
}

// // GetPublicKey derives the public key from the signing key
// func GetPublicKey(signingKey *fr.Element) *bn254.G2Affine {
// 	return bls.NewKeyPair(signingKey).PK
// }

func Sign(signingKey *fr.Element, messageHex string) (string, error) {
	domain := getDomain()
	messagePoint, err := hashToPoint(messageHex, domain)
	if err != nil {
		return "", err
	}
	...
	signingKey.BigInt(&scalarBigInt)
	sig := new(bn254.G1Affine)
	sig.ScalarMultiplication(messagePoint, &scalarBigInt)
	xBytes := sig.X.Bytes()
	yBytes := sig.Y.Bytes()

	sigObj := map[string]string{
		"x": "0x" + hex.EncodeToString(xBytes[:]),
		"y": "0x" + hex.EncodeToString(yBytes[:]),
	}

	// 5. JSON.stringify-style output
